...
{ 
	...
	char bufSave[BUFFER_SZ];				//buffer for saving off message read in 
	...
	*/
	...
	/************************************************************/
	int flag = 0;   //flag to set encryption method 0:unencrpted 1:AES 2:DES 3:3DES
		...
		{	  
		  ...
		  if ( !SharedKeyISCalculated)
		  {
			...
			if (flag == 0)
			{
			    ...
			    if (strcmp(buf,"AES\n") == 0)
			      flag = 1;
			    else if (strcmp(buf, "DES\n") == 0) 
			      flag = 2;
			    else if (strcmp(buf, "3DES\n") == 0) 
				  flag = 3;
			 
				printf("After strcmp buf with type flag = %d\n", flag);
			...
			{ 
				...
				//printf("Right before make call to AES encrypt\n");
			...
			else if (flag == 2)		//DES encryption
			{
				...
				printf("Place holder for encryption with DES\n");
			...
			else if ( flag == 3) //3DES enryption
			{
				...
				printf("Place holder for encryption with 3DES\n");
		...
		if (FD_ISSET(sd, &masterfds)) 
		{
		  ...
		  if (flag == 0)
		  {
			...
			if (valuerd > 0) //output the message that was read from buffer
		    {
				...
				//printf("Contents of buf after toupper %s\n", buf);
		  ...
		  { 
			...
			if (valuerd > 0) //AES decrypt and output the message that was read from buffer
		    {
				//printf("Received encrypted text is:\n");
				//printf("%s\n", buf);
				
				int buf_len = size(&buf[0]);
				CharToUnsignedChar(buf, ciphertext, buf_len); 
				//printf("inside FD_ISSET ready to read BEFORE make call to AES decrypt\n");
				decryptedtext_len = decrypt (ciphertext, /*ciphertext_len*/buf_len, key, iv, decryptedtext);
				//printf("inside FD_ISSET ready to read AFTER make call to AES decrypt\n");
				...
				printf("before decryptedtext to char buf\n");
				bzero(buf, BUFFER_SZ);
				//buf = new char;
				unsignedCharToChar(decryptedtext, buf, decryptedtext_len);
				int length = strlen(buf);
				printf("length of decrypted buf %d\n", length);
